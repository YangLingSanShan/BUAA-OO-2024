...
public class Output {
    public String getOutPut(HashMap<Factor, TreeMap<BigInteger, BigInteger>> res) {
        String result = "";
        for (Factor factor : res.keySet()) {
            TreeMap<BigInteger, BigInteger> fun = res.get(factor);
            if (factor.getType().equals("Const")) {
                ...
                for (BigInteger i : fun.keySet()) {    //sec*x^fir   fun.get(i) 系数     i 指数
                    HashMap<Factor, TreeMap<BigInteger, BigInteger>> fac = factor.getExpression();
    ...
    private String getMonomial(String res, TreeMap<BigInteger, BigInteger> fun, BigInteger i) {
        String result = res;
    ...
    private String getExpExpression(String res, String inEX, TreeMap<BigInteger,
            BigInteger> fun, BigInteger i) {
        String result = res;
        String appendix = getString(inEX);
    ...
    private static String getString(String inEX) {
    ...
    public String Consolidate(String s) {
        Lexer lexer = new Lexer(shorten(s.replace(" ", "").replace("\t", "")));
        Parser parser = new Parser(lexer, null);
        Expr expr = parser.parserExpr();
