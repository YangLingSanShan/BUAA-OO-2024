...
public class Expr extends Factor {
    ...
    public HashMap<Factor, TreeMap<BigInteger, BigInteger>> getExpression() {
        ...
        HashMap<Factor, TreeMap<BigInteger, BigInteger>> res = c.getExpression();
    ...
    public Factor differentiate() {
        ...
        for (Term term : terms) {
            ArrayList<Term> temp = term.differentiate();
            if (!temp.isEmpty()) {
                termList.addAll(temp);
            }
        }
        if (!termList.isEmpty()) {
            return new Expr(termList);
        } else {
            return new Const(BigInteger.valueOf(0));
        }
    }
