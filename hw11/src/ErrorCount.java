...
public class ErrorCount {
    ...
    private int einfNum = 0;
    private HashMap<Integer, Integer> einf = new HashMap<>();
    private int eeiNum = 0;
    private HashMap<Integer, Integer> eei = new HashMap<>();
    private int emiNum = 0;
    private HashMap<Integer, Integer> emi = new HashMap<>();
    private int minfNum = 0;
    private HashMap<Integer, Integer> minf = new HashMap<>();
    ...
    public int getEinfNum() {
        return einfNum;
    }

    public int getEeiNum() {
        return eeiNum;
    }
    ...
    public int getEinfIdTriggerTime(int id) {
        return einf.get(id);
    }

    public int getEeiIdTriggerTime(int id) {
        return eei.get(id);
    }

    public int getEmiIdTriggerTime(int id) {
        return emi.get(id);
    }

    public int getMinfIdTriggerTime(int id) {
        return minf.get(id);
    }
    ...
    public void triggerEmojiIdNotFoundException(int id) {
        einfNum++;
        if (einf.containsKey(id)) {
            einf.replace(id, einf.get(id) + 1);
        } else {
            einf.put(id, 1);
        }
    }

    public void triggerEqualEmojiIdException(int id) {
        eeiNum++;
        if (eei.containsKey(id)) {
            eei.replace(id, eei.get(id) + 1);
        } else {
            eei.put(id, 1);
        }
    }

    public void triggerEqualMessageIdException(int id) {
        emiNum++;
        if (emi.containsKey(id)) {
            emi.replace(id, emi.get(id) + 1);
        } else {
            emi.put(id, 1);
        }
    }

    public void triggerMessageIdNotFoundException(int id) {
        minfNum++;
        if (minf.containsKey(id)) {
            minf.replace(id, minf.get(id) + 1);
        } else {
            minf.put(id, 1);
        }
    }
