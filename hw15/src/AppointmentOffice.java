...
public class AppointmentOffice implements Server {
    ...
    public AppointmentOffice(HashMap<String, Person> people) {
        this.people = people;
    }
        ...
        while (iterator.hasNext()) {
            ...
            if (date.isBefore(cleanDate)) {
                ...
                for (String id: map.keySet()) {
                    HashMap<LibraryBookId, Integer> books = map.get(id);
    ...
    public boolean personhasBookB(String studentId) {
        for (HashMap<String, HashMap<LibraryBookId, Integer>> map: waitingBooks.values()) {
            if (map.containsKey(studentId)) {
                HashMap<LibraryBookId, Integer> books = map.get(studentId);
                for (LibraryBookId id: books.keySet()) {
                    if (id.isTypeB() && books.get(id) > 0) {
                        return true;
                    }
                }
            }
